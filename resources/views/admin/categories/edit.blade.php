<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <h1 class="text-xl font-bold text-slate-900">Edit Kategori: <span class="text-alazhar">{{ $category->name }}</span></h1>
        </div>
    </x-slot>
                ...
                <div class="p-8">
                    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input type="text" name="name" id="name" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:ring-alazhar focus:border-alazhar transition-colors" value="{{ old('name', $category->name) }}" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="description" :value="__('Deskripsi Singkat')" class="text-slate-700 font-semibold mb-1" />
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:ring-alazhar focus:border-alazhar transition-colors">{{ old('description', $category->description) }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div class="mb-8 p-4 bg-slate-50 rounded-lg border border-slate-100 flex items-start">
                            <div class="flex items-center h-5">
                                <input type="checkbox" name="is_hidden" id="is_hidden" value="1" {{ old('is_hidden', $category->is_hidden) ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-alazhar shadow-sm focus:ring-alazhar focus:ring-offset-0 transition-colors">
                            </div>
