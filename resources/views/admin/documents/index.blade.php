                    ...
                    <table class="w-full text-sm text-left text-slate-600">
                        ...
                        <tbody id="sortable-table-body">
                            @forelse ($documents as $document)
                                <tr class="bg-white border-b border-slate-100 hover:bg-blue-50/50 transition-colors duration-200" data-id="{{ $document->id }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-slate-900 line-clamp-1">{{ $document->title }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        @if($document->is_published)
                                            <span class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700 border border-green-200">Publik</span>
                                        @else
                                            <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600 border border-slate-200">Draft</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                        <a href="{{ route('admin.documents.show', $document->id) }}" class="inline-flex justify-center items-center w-8 h-8 rounded-md bg-white border border-slate-200 text-slate-500 hover:text-alazhar hover:border-alazhar hover:bg-blue-50 transition-all shadow-sm" title="Lihat">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </a>
                                        <a href="{{ route('admin.documents.edit', $document->id) }}" class="inline-flex justify-center items-center w-8 h-8 rounded-md bg-white border border-slate-200 text-alazhar hover:text-white hover:bg-alazhar transition-all shadow-sm" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form action="{{ route('admin.documents.destroy', $document->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus artikel ini?');">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p class="text-base font-medium text-slate-500">Belum ada artikel/dokumen.</p>
                                            <p class="text-sm">Silakan buat artikel pertama Anda.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                onEnd: function (evt) {
                    ...
                    .then(data => {
                        if(data.success) {
                            ...
                            toast.innerHTML = '<svg class="w-5 h-5 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg><span>Urutan artikel diperbarui</span>';
