                    ...
                    <form action="{{ route('admin.documents.update', $document->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div class="md:col-span-2">
                                ...
                                <x-text-input type="text" name="title" id="title" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:ring-alazhar focus:border-alazhar transition-colors" value="{{ old('title', $document->title) }}" required autofocus />
                        ...
                        <div class="mb-6">
                            ...
                            <div class="border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                                <textarea name="content" id="content-editor" class="mt-1 block w-full border-0">{{ old('content', $document->content) }}</textarea>
                            </div>
                        ...
                        <div class="mb-8 p-4 bg-slate-50 rounded-lg border border-slate-100 flex items-start">
                            <div class="flex items-center h-5">
                                <input type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $document->is_published) ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-300 text-alazhar shadow-sm focus:ring-alazhar focus:ring-offset-0 transition-colors">
                            </div>
                            <div class="ml-3 text-sm">
                                <label for="is_published" class="font-medium text-slate-700">Terbitkan Langsung</label>
                                <p class="text-slate-500">Jika tidak dicentang, artikel akan disimpan sebagai <em>Draft</em> dan ditarik dari halaman pengunjung.</p>
                            </div>
                        </div>
